...
def call_ai(prompt, system=None):
    ...
    try:
        ...
        api_key = prov.get("api_key") or cfg.get("openrouter", {}).get("api_key") or cfg.get("api_key", "")
        model = prov.get("default_model") or prov.get("model") or cfg.get("openrouter", {}).get("default_model") or "poolside/laguna-xs-2.1:free"
        ...
        try:
            ...
            if "error" in data:
                ...
                if isinstance(err, dict):
                    last_err = f"[{m}] {err.get('message', err)}"
                else:
                    last_err = f"[{m}] {err}"
                if DEBUG_AI:
